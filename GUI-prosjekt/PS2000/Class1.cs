...
namespace PS2000GUI
{
    ...
    public interface IPSU
    {
        ...
        public double Data(byte[] bytesToSend);
    ...
    public class PSU : IPSU
    {
        ...
        public double GetCurrent()
        {
            ...
            double current = Data(bytesToSend);
            return current;
        }
        ...
        using (SerialPort port = new SerialPort(comport, 115200, 0, 8, StopBits.One))
        {
            ...
            if (Serialresponse[2] == 1) // means that I got a response on obj, which is refers to the object list.
            {
                ...
                Console.WriteLine($"Serial number: {serialNumberString}");
        ...
        public string GetRemote()
        {
            ...
            using (SerialPort port = new SerialPort(comport, 115200, 0, 8, StopBits.One))
            {
                ...
                if (RCresponse[3] == 0)
                {
                    ...
                    remoteControl = string.Format("Remote Control is turned on");
                    return remoteControl;
                }
                else
                {
                    Console.WriteLine(String.Format("Remote control is not turned on due to error: {0}", RCresponse[3].ToString()));
                    remoteControl = string.Format("Remote control is not turned on due to error: {0}", RCresponse[3].ToString());
                    return remoteControl;
                }
            }
        }
